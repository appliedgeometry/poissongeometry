...
class PoissonGeometry:
    ...
    def coordinates(self):
        """ Print the coordinates to use in all functions according to
        the dimension 'dim'.

        :return: a string type variable. For example,
            * The coordinates are (x1, x2, x3).
        Or
            * The coordinates are (x1,...,x5)
        """
        if self.dim < 4:
    ...
    def kernel_p_test(self, bivector, one_form):
        ...
        :return: a boolean type variable. For the previous example,
            'True'
        """
    ...
    def one_forms_bracket(self, bivector, one_form_1, one_form_2):
        ...
        for z in itools.combinations(range(1, self.dim + 1), 2):
            ii_sharp_alpha_d_beta[z[0] - 1] = sy.simplify(
                ii_sharp_alpha_d_beta[z[0] - 1] +
                self.p_sharp(bivector, one_form_1)[z[1]] * (
                            sy.diff(one_form_2[z[0]],
                                    self.x_coord[z[1] - 1]) - sy.diff(
                        one_form_2[z[1]], self.x_coord[z[0] - 1])))
            ii_sharp_alpha_d_beta[z[1] - 1] = sy.simplify(
                ii_sharp_alpha_d_beta[z[1] - 1] +
                self.p_sharp(bivector, one_form_1)[z[0]] * (
                            sy.diff(one_form_2[z[1]],
                                    self.x_coord[z[0] - 1]) - sy.diff(
                        one_form_2[z[0]], self.x_coord[z[1] - 1])))
            ii_sharp_beta_d_alpha[z[0] - 1] = sy.simplify(
                ii_sharp_beta_d_alpha[z[0] - 1] +
                self.p_sharp(bivector, one_form_2)[z[1]] * (
                            sy.diff(one_form_1[z[0]],
                                    self.x_coord[z[1] - 1]) - sy.diff(
                        one_form_1[z[1]], self.x_coord[z[0] - 1])))
            ii_sharp_beta_d_alpha[z[1] - 1] = sy.simplify(
                ii_sharp_beta_d_alpha[z[1] - 1] +
                self.p_sharp(bivector, one_form_2)[z[0]] * (
                            sy.diff(one_form_1[z[1]],
                                    self.x_coord[z[0] - 1]) - sy.diff(
    ...
    def quadratic_normal_form_R3(self, bivector):
        ...
        """
        x_aux = sy.symbols(f'x1:{4}')
        mtx_mod_vf_biv = sy.Matrix((sy.derive_by_array(
            self.modular_vf(bivector, 1)[1], x_aux), sy.derive_by_array(
            self.modular_vf(bivector, 1)[2], x_aux), sy.derive_by_array(
            self.modular_vf(bivector, 1)[3], x_aux)))
        egval_mod_vf = mtx_mod_vf_biv.eigenvals()
        # All multiplicity are 1
        if mtx_mod_vf_biv == sy.zeros(3):
            # print(f'Case I')
            return bivector
        if reduce((lambda a, b: a * b), list(egval_mod_vf.values())) == 1:
            # Three real eigenvalues
            if all(sy.im(t) == 0 for t in list(egval_mod_vf.keys())) == bool(
                    1):
                # Nonzero eigenvalues
                if reduce((lambda c, d: c * d),
                          list(egval_mod_vf.keys())) != 0:
                    # print(f'Case II', egval_mod_vf)
                    return {12: sy.sympify(f'(1/3*(b1 - b2) - a)*x1*x2'),
                            13: sy.sympify(f'(1/3*(b1 - b3) + a)*x1*x3'),
                            23: sy.sympify(f'(1/3*(b2 - b3) - a)*x2*x3')}
                # One zero eigenvalue
                else:
            ...
            # Two complex and one real eigenvalues
                ...
                if reduce((lambda c, d: c * d),
                    ...
                    return {12: sy.sympify(f'-(a + 1/3*b2)*(x1**2 + x2**2)'),
                            13: sy.sympify(f'(b1*x1 + (2*a - 1/3*b2)*x2)*x3'),
                            23: sy.sympify(f'(b1*x2 - (2*a - 1/3*b2)*x1)*x3')}
                # One zero eigenvalue
                else:
        ...
        if reduce((lambda a, b: a * b), list(egval_mod_vf.values())) == 2:
            # Diagonalizable case
            if mtx_mod_vf_biv.is_diagonalizable(reals_only=True) == bool(1):
                print(f'Case IV')
            # Non-diagonalizable case
            else:
                # print(f'Case VII')
                return {12: sy.sympify(f'(1/3 - a)*x2**2'),
                        13: sy.sympify(f'(b*x1 + (2*a + 1/3)*x2)*x3'),
                        23: sy.sympify(f'b*x2*x3')}
    ...
    def gauge_transformation(self, bivector, two_form):
        two_form_B = sy.MatrixSymbol('B', self.dim, self.dim)
        two_form_B = sy.Matrix(two_form_B)
        ...
        for z in itools.combinations_with_replacement(range(1, self.dim + 1),
            ...
            if z[0] == z[1]:
                ...
                two_form_B[z[0] - 1, z[1] - 1] = sy.sympify(
                    two_form[int(''.join(str(i) for i in z))])
                two_form_B[z[1] - 1, z[0] - 1] = (-1) * two_form_B[
                    z[0] - 1, z[1] - 1]
        I_minus_BP = sy.Matrix(sy.simplify(
            sy.eye(self.dim) - two_form_B * self.p_matrix(bivector)))
        det_I_minus_BP = sy.factor(sy.simplify(I_minus_BP.det()))
        if det_I_minus_BP == 0:
            return f'\nThe differential 2-form can\'t induces a gauge ' \
                f'transformation.'
        else:
            gauge_mtx = sy.Matrix(
                sy.simplify(self.p_matrix(bivector) * (I_minus_BP.inv())))
            return gauge_mtx, sy.sympify(det_I_minus_BP)

# ------ CLASSIFICATION -------------------------------------------------------

    def lie_der_vf_2_form(self, vector_field, two_form):
        for i in vector_field:
            vector_field[i] = sy.sympify(vector_field[i])
        for key in two_form:
            two_form[key] = sy.sympify(two_form[key])
        lie_der_2_form = {}
        for key in two_form:
            lie_der_aux_k = sy.simplify(sum(
                vector_field[k] * sy.diff(two_form[key], self.x_coord[k - 1])
                for k in range(1, self.dim + 1)))
            ky2 = tuple(int(i) for i in str(key))
            lie_der_aux_ki = sy.simplify(sum(
                (-1) * sy.diff(vector_field[k], self.x_coord[ky2[1] - 1]) *
                two_form[int(''.join((f'{k}', f'{ky2[0]}')))] for k in
                range(1, ky2[0])))
            lie_der_aux_ik = sy.simplify(sum(
                sy.diff(vector_field[k], self.x_coord[ky2[1] - 1]) * two_form[
                    int(''.join((f'{ky2[0]}', f'{k}')))] for k in
                range(ky2[0] + 1, self.dim + 1)))
            lie_der_aux_kj = sy.simplify(sum(
                sy.diff(vector_field[k], self.x_coord[ky2[0] - 1]) * two_form[
                    int(''.join((f'{k}', f'{ky2[1]}')))] for k in
                range(1, ky2[1])))
            lie_der_aux_jk = sy.simplify(sum(
                (-1) * sy.diff(vector_field[k], self.x_coord[ky2[0] - 1]) *
                two_form[int(''.join((f'{ky2[1]}', f'{k}')))] for k in
                range(ky2[1] + 1, self.dim + 1)))
            lie_der_2_form.setdefault(key, sy.simplify(
    ...
    def ext_der_two_form(self, two_form):
        ...
        for z in itools.combinations(range(1, self.dim + 1), 3):
            d_2_form_aux = sy.simplify(sy.simplify(sy.diff(
                two_form[int(''.join((f'{z[1]}', f'{z[2]}')))],
                self.x_coord[z[0] - 1])) + sy.simplify((-1) * sy.diff(
                two_form[int(''.join((f'{z[0]}', f'{z[2]}')))],
                self.x_coord[z[1] - 1])) + sy.simplify(sy.diff(
                two_form[int(''.join((f'{z[0]}', f'{z[1]}')))],
                self.x_coord[z[2] - 1])))
            d_2_form.setdefault(int(''.join(str(i) for i in z)),
                                sy.simplify(d_2_form_aux))
        return d_2_form
    ...
    def inner_euler_vf_2_form(self, two_form):
        ...
        for i in range(1, self.dim + 1):
            ...
            ii_E_two_form.setdefault(i, sy.simplify(ii_E_2_form_ji + ii_E_2_form_ij))
        return ii_E_two_form

    def inner_euler_vf_2_form_2(self, two_form):
        form = sy.MatrixSymbol('O', self.dim, self.dim)
        form = sy.Matrix(form)
        # Assigns the corresponding coefficients of the 2-form
        for z in itools.combinations_with_replacement(range(1, self.dim + 1),
                                                      2):
            if z[0] == z[1]:
                ...
                form[z[0] - 1, z[1] - 1] = sy.sympify(
                    two_form[int(''.join(str(i) for i in z))])
                form[z[1] - 1, z[0] - 1] = (-1) * form[
                    z[0] - 1, z[1] - 1]
        euler_vf = sy.Matrix(self.x_coord)
